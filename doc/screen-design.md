...
| ボタン | 動作 |
|--------|------|
| インポート | インポートモーダル (M1) を開く |
| ダウンロード | ダウンロードモーダル (M2) を開く |
| + | `/combo/new` に遷移 |
...
| 削除 | 展開を削除してホーム画面に戻る | 編集時のみ活性 (新規作成時は非活性) |
...
配置済み画像をクリックすると、バッジ選択肢を表示:

| 選択肢 | 動作 |
|--------|------|
| チェーン | 連番 (丸数字 1〜20) を画像上に SVG で表示。番号は追加順で自動採番。削除時は振り直し |
| 削除 | 画像をセルから除去 |
| 閉じる | バッジ選択肢を閉じる |
