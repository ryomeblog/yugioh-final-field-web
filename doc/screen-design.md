...
| 5 | 共有展開画面 | `/share?d=<encoded>` | URL共有された展開の閲覧 (読み取り専用、IndexedDB不要) |
...
| 画面 | ステップ数 | 画像ファイル |
|------|-----------|-------------|
| ホーム | 6 | home-1.png, home-2.png (2枚目以降共通) |
| 展開作成・編集 | 8 | edit-1.png 〜 edit-7.png |
...
| 要素 | 説明 |
|------|------|
| ヘッダ | 戻る矢印 (ホーム画面へ)、展開タイトル |
| 初動札 | 画像サムネイルを横並び表示 (読み取り専用) |
| ステップカード | 展開詳細画面と同じレイアウト |

- URLのクエリパラメータ `d` からデータを復元して表示
- IndexedDB を使用しない (URLのデータのみで完結)
- 編集・削除・ダウンロード機能なし
- 画像は `externalUrl` のみ対応 (blob画像は共有不可)
- デコードに失敗した場合はエラーメッセージとホームへのリンクを表示
