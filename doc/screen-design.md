...
| 要素 | 説明 |
|------|------|
| ヘッダ | タイトル「展開一覧」、インポート / ダウンロード / + / 設定ボタン |
...
各画面を初回表示した際にチュートリアルオーバーレイが表示される。

### 仕様

- 画面の要素をスポットライト (SVGマスクによるくり抜き) でハイライトし、横に解説テキストを表示
- 「次へ」ボタンで次のステップに進み、「スキップ」ボタンまたは背景クリックで終了
- 完了状態は `localStorage` に画面単位で保存
- 設定モーダル (M5) で完了済みチュートリアルをリセットして再表示可能

### チュートリアル対象画面

| 画面 | ステップ数 | ハイライト対象 |
|------|-----------|--------------|
| ホーム | 5 | 展開一覧、インポート、ダウンロード、+ボタン、設定 |
| 展開作成・編集 | 5 | タイトル入力、初動札、ステップ追加、画像一覧、保存 |
| 展開詳細 | 4 | 初動札、ステップ一覧、編集ボタン、ダウンロード |

### 設定モーダル (M5)

- ホーム画面ヘッダの歯車ボタンから開く
- 各画面のチュートリアルを「リセット」ボタンで再表示可能に
- 未完了の場合は「未完了」ラベルを表示
